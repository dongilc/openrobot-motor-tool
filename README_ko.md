...
OpenRobot 모터 컨트롤러용 GUI 도구입니다. **CAN-Only 아키텍처** — PCAN-USB를 통해서만 통신합니다.
...
> **참고**: 본 도구는 CAN-Only 아키텍처를 사용하며, USB-to-CAN 어댑터 없이는 동작하지 않습니다. PCAN 드라이버를 별도 설치해야 합니다.

## 빠른 시작

1. [Releases](https://github.com/dongilc/openrobot-motor-tool/releases/latest) 에서 `OpenRobot_Motor_Tool_v3.4.exe` 다운로드
2. PEAK-System에서 PCAN 드라이버 설치
3. PCAN-USB 어댑터를 PC에 연결
4. 실행 파일 실행
5. **Open** 클릭 → 자동 CAN 버스 스캔 시작
